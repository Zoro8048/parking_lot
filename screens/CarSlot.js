...
var slot= '';
class CarSlot extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            showForm: false,
    ...
    render() {
        ...
        slots.forEach(val => 
            {
                if(this.props.parkList.length !=0 && this.props.parkList.find(a => a.includes(val.slot))){
                    parked.push(val.slot);
                }
            });
        return(
            <View style={{flex: 1}} >
                ...
                {this.state.showForm && <Form _closeForm={this._closeForm} slot={slot} type="Car" />}
            </View>
        )
    }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(CarSlot);